...
import net.earelin.boxes.reader.ValueReaderFactory;
        ...
        if (!columnMappersCache.containsKey(canonicalName)) {
            columnMappersCache.put(canonicalName, Arrays.stream(type.getDeclaredFields())
                    ...
                    .map(field -> {
                        ...
                        var reader = ValueReaderFactory.createValueReader(field.getType(), column.start(), column.length());
                        var setter = getFieldSetter(type, field);
                        return new ColumnMapper(reader, setter);
                    })
                    .toList());
